...
class transformation:
    ...
    def transformer(self, df):
        ...
        if self.dateCirculationTo == "age":
            ...
            df_transformer['date_circ'] = pd.to_datetime(df_transformer['date_circ'], format='%d/%m/%Y')
            df_transformer['annee'] = df_transformer['date_circ'].dt.year

        # suppression de la colonne date_circulation
        df_transformer = df_transformer.drop(
            columns=['date_circ'], axis=1)

        # transformation de la colonne marque en one hot encoding
        transformedMarque = self.OHE.transform(
            df_transformer[['marque']]).toarray()
        df_transformer["Marque_"+self.OHE.categories_[0]] = transformedMarque

        # transformation de la colonne couleur en one hot encoding
        transformedCouleur = self.OHE1.transform(
            df_transformer[['couleur']]).toarray()
        df_transformer["Couleur_" +
                       self.OHE1.categories_[0]] = transformedCouleur
